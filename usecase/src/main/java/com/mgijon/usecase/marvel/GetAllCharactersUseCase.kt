...
class GetAllCharactersUseCase @Inject constructor(private val marvelRepository: MarvelRepository) : MarvelBaseUseCase() {

    operator fun invoke(): Flow<Resource<List<Character>>> =
        getFlow({ Resource.Success(marvelRepository.getAll()) })
}