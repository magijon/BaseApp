...
abstract class BaseViewModel : ViewModel() {
    ...
    fun setState(resource: Resource<*>, stateBase: StateBase) {
        when (resource) {
            is Resource.Success -> _state.value = stateBase
            is Resource.Error -> _state.value = StateBase.ErrorStateBase(
                ErrorException.BaseErrorExceptionMessage(
                    messageString = resource.message ?: "An unexpected error occurred!"
                )
            )
            is Resource.Loading -> _state.value = StateBase.LoadingStateBase
        }
    }
}